...
param (
	...
	$Role = "WebRole",								#Select role to assign DSC
...
#Validate if the subscription is Internal or Production
$exists = get-azurermresourcegroup -Name $MgmtRGName -ErrorAction silentlycontinue

if (-not($exists)){$MgmtRGName = "del-mgt-rg"}
...
foreach ($VM in $targetVMs)
{
	...
	AssignDSCConfig $VM.Name $VM.ResourceGroupName $DSCAccountName "DSC$($EnvironmentName).$($Hostname)"                          
}
